...
streznik.get('/', function(zahteva, odgovor) {
    if(zahteva.session.loggedInClient==undefined || zahteva.session.loggedInClient==null){
  ...
  form.parse(zahteva, function(napaka2, polja, datoteke){
    pesmiIzRacuna(polja.seznamRacunov, function(pesmi){
      strankaIzRacuna(polja.seznamRacunov, function(stranka){
        ...
        odgovor.render('eslog', {vizualiziraj: true, postavkeRacuna: pesmi, stranka: narocnik});
      });
    });
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.login = polja.seznamStrank;
    var napaka2 = false;
    try {
      var stmt = pb.prepare("\
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.loggedInClient=polja.seznamStrank;
    odgovor.redirect('/')
  });
})
...
streznik.post('/odjava', function(zahteva, odgovor) {
  zahteva.session.loggedInClient=null;
  odgovor.redirect('/prijava')
})
